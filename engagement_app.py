# ============================================================
# engagement_app.py — Classroom Engagement Analyzer
...
class TemperatureScaler(nn.Module):
    """Post-hoc calibration via learned temperature parameter."""
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        return torch.sigmoid(
            torch.logit(x.clamp(1e-6, 1 - 1e-6)) / self.temperature)
...
def load_model():
    import __main__
    # Register TemperatureScaler so pickle can find it when loading
    ...
    m         = HybridImLNBiLSTM(input_dim=input_dim)
    ...
    if os.path.exists("calibrator.pkl"):
        try:
            ...
            st.warning(f"Calibrator could not load ({e}). Running without calibration.")
            cal = None

    return m, scaler, cal, input_dim, seq_len
...
def build_sequence(raw_frames, scaler, seq_len=30):
    """
    Convert list of per-frame 22-dim feature vectors into a
    normalized (seq_len, 44) sequence with delta features.
    ...
    full_seq = np.concatenate([seq, delta], axis=1)   # (seq_len, 44)

    N, F      = full_seq.shape
    flat_norm = scaler.transform(full_seq.reshape(-1, F))
    return flat_norm.reshape(N, F).astype(np.float32)


def infer(sequence, model, cal):
    """
    Run inference on a (seq_len, 44) numpy array.
    Applies temperature scaling calibration if available.
    ...
    with torch.no_grad():
        ...
        if cal is not None:
            raw = cal(raw)
        return float(np.clip(raw.item(), 0, 1))


def extract_video_sequence(path, detector, scaler, seq_len=30):
    """Extract a (seq_len, 44) feature sequence from an uploaded video."""
    cap, idx, raw_frames = cv2.VideoCapture(path), 0, []
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        if idx % 5 == 0:
            ...
            raw_frames.append(
                lm_to_feat(result.face_landmarks[0])
    ...
    if len(raw_frames) < 2:
        return None
    return build_sequence(raw_frames, scaler, seq_len)
...
def get_level(score):
    ...
    return ("Poor", "#ef4444", "#b91c1c", "Very low engagement detected.")
...
with col_header:
    ...
    <div style="display:flex;align-items:center;gap:0.5rem">
        <div style="font-family:'Outfit',sans-serif;font-size:1.6rem;
...
st.markdown('<div class="rule" style="margin-top:1.5rem;margin-bottom:2rem"></div>',
            unsafe_allow_html=True)
...
if st.session_state.page == "upload":
    ...
    if uploaded and run:
        ...
        try:
            model, scaler, cal, input_dim, seq_len = load_model()
            det = load_detector()
        ...
        with st.spinner("Processing..."):
            sequence = extract_video_sequence(tmp_path, det, scaler, seq_len)
        ...
        if sequence is None:
            ...
            score              = infer(sequence, model, cal)
            ...
            <div class="result-card">
                ...
                <div class="result-level" style="color:{txt_c}">{lbl}</div>
            ...
            for _, lvl_lbl, color, _, desc in LEVELS:
                ...
                <div class="scale-row">
                  <div class="scale-left">
                    <div class="scale-dot" style="background:{color}"></div>
...
# ============================================================
    ...
    st.markdown(
        f'<div class="live-stat">Updating every {every} frames</div>',
        unsafe_allow_html=True)
    ...
    if st.session_state.running:
        ...
        face_buf    = {i: deque(maxlen=seq_len * 3) for i in range(4)}
        ...
        while st.session_state.running:
            ...
            for i in range(nf):
                ...
                if fc % every == 0 and len(face_buf[i]) >= 2:
                    raw_list      = list(face_buf[i])
                    sequence      = build_sequence(raw_list, scaler, seq_len)
                    face_scores[i] = infer(sequence, model, cal)

                    row = {"time": time.strftime("%H:%M:%S"), "frame": fc}
                    for j in range(nf):
                        row[f"s{j+1}"] = (
            ...
            for i, col in enumerate(scols):
                ...
                if len(vals):
                    ...
                    with cols[i]:
                        st.markdown(f"""
                        <div class="sum-card">
                            <div class="section-label">Student {i+1}</div>
                            <div class="sum-level" style="color:{tc}">
                                {lbl}
                            </div>
